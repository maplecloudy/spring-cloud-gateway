...
import org.springframework.cloud.gateway.config.DynamicRoutes;
...
public class RoutePredicateHandlerMapping extends AbstractHandlerMapping {
  ...
  public DynamicRoutes getDynamicRoutes() {
    return (DynamicRoutes) this.getApplicationContext()
        .getBean("dynamicRoutes");
  }
  ...
  private static Integer getPortProperty(Environment environment,
      String prefix) {
  ...
  protected Mono<Route> lookupRoute(ServerWebExchange exchange) {
    ...
    Mono<Route> routeRt = this.processRoutes(this.routeLocator.getRoutes(),
        exchange);
    // individually filter routes so that filterWhen error delaying is not a
    // problem
    DynamicRoutes dynamicRoutes = this.getDynamicRoutes();
    if (dynamicRoutes != null) {
      routeRt = routeRt.switchIfEmpty(
          this.processRoutes(dynamicRoutes.getRoutes(exchange), exchange));
    }
    return routeRt;
    
    /*
     * TODO: trace logging if (logger.isTraceEnabled()) {
     * logger.trace("RouteDefinition did not match: " +
     * routeDefinition.getId()); }
     */
  }
  
  private Mono<Route> processRoutes(Flux<Route> routes,
      ServerWebExchange exchange) {
    return routes.concatMap(route -> Mono.just(route).filterWhen(r -> {
      ...
      return r.getPredicate().apply(exchange);
        ...
        // swallow it
        .doOnError(e -> logger
            .error("Error applying predicate for route: " + route.getId(), e))
